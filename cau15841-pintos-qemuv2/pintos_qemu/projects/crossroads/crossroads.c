...
void run_crossroads(char **argv)
{
	...
	for (i=0; (size_t) i<strlen(argv[1]); i++) {
		if (argv[1][i] == ':') {
			thread_cnt++; // 필요한 스레드 개수 세기
		}
	}
	printf("initializing %d vehicles...\n", thread_cnt);
	vehicle_info = malloc(sizeof(struct vehicle_info) * thread_cnt); // 개수 만큼 스레드 정보 struct 만들기
	parse_input(vehicle_info, argv[1]);

	for (i=0; i<thread_cnt; i++) {
		/* put map locks */
		vehicle_info[i].map_locks = map_locks; // 각 스레드에 공통된 map lock 제공 (by pointer)
	}
